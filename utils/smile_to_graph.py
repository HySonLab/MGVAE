...
class GGNNPreprocessor(object):
    ...
    def get_ogb_features(self, mol, smiles):
        type_check_num_atoms(mol, self.max_atoms)
        graph = smiles2graph(smiles)
        #num_node = graph["num_node"]
        atom_array = self.pad_zeros_features(graph["node_feat"])
        adj_array = construct_discrete_edge_matrix(mol, out_size=self.out_size)
        return atom_array, adj_array

    def pad_zeros_features(self, features):
        #print(features.shape)
        if features.shape[0] == self.max_atoms:
            return features
        num_atomic_features = features.shape[1]
        zeros = numpy.zeros((self.max_atoms - features.shape[0], num_atomic_features))
        return numpy.concatenate((features, zeros), axis = 0)

    def pad_zeros_adj(self, adj):
        if adj.shape[0] == self.max_atoms:
            return adj
        N = adj.shape[0]
        zeros = torch.zeros(self.max_atoms - N, N)
        adj = torch.cat((adj, zeros), dim = 0)
        zeros = torch.zeros(self.max_atoms, self.max_atoms - N)

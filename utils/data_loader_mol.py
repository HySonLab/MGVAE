## Adpated from GDSS: https://github.com/harryjo97/GDSS.git
...
def get_transform_fn(dataset):
    if dataset == 'QM9':
        def transform(data):
            ...
            # 6: C, 7: N, 8: O, 9: F
            qm9_atomic_num_list = [6, 7, 8, 9]
            x_ = np.zeros((9, 4), dtype=np.float32)
            num_mol = 0
            for i in range(9):
                if x[i] != 0:
                    ind = qm9_atomic_num_list.index(x[i])
                    x_[i, ind] = 1.
                    num_mol += 1
            x_ = torch.tensor(x_).to(torch.float32)
            x_[num_mol:, :] = 0
            x_label = x_.argmax(1)
            x_label[num_mol:] = 100 # ignore index

            adj_cloned = copy.deepcopy(adj)
            edge = torch.from_numpy(adj_cloned.argmax(0))
            edge[num_mol : , :] = 100 # ignore index
            edge[:, num_mol : ] = 100 # ignore index
           
            return x_, torch.from_numpy(adj), edge.long(), x_label.long()
        ...
        def transform(data):
            ...
            zinc250k_atomic_num_list = [6, 7, 8, 9, 15, 16, 17, 35, 53]
...
def dataloader(config, get_graph_list=False):
    print("Load molecular graphs dataset")
    start_time = time()

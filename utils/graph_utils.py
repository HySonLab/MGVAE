## Adapted from https://github.com/harryjo97/GDSS.git
...
def node_flags(x, eps=1e-5):
    flags = torch.abs(x).sum(-1).gt(eps).to(dtype = torch.float32)
    return flags

# -------- Create flags tensor from graph dataset --------
def node_flags_1(adj, eps=1e-5):
    #print(adj.shape)
